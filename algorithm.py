...
def check_rising_edge():
    ...
    if len(pm25_values) >= WINDOW_SIZE and all(float(timestamp) >= one_hour_ago for timestamp in timestamp_values):
        threshold = 1.25
    ...
    try:
        cursor.execute('''INSERT INTO detectiontestV2 (key, timestamp, pm25, baselinepm25, relaystate) 
                          VALUES (?, ?, ?, ?, ?)''', (1, int(time.time()), pm25_values[0], baseline_pm25, relay_state))
    except sqlite3.Error as e:
